...
export default function StatusFilter() {
	...
	const handleSearchParams = useCallback(
		(debouncedValue) => {
			...
			if (debouncedValue.length > 0) {
				params.set("resignationStatus", debouncedValue);
			} else {
				params.delete("resignationStatus");
			}
	...
	useEffect(() => {
		...
		const searchQuery = params.get("resignationStatus") ?? [];
	...
	return (
		<div className="flex flex-col gap-2 w-full">
			<FormControl sx={{ m: 1, width: 300 }}>
				<InputLabel id="resignationStatus-multiple-checkbox-label">
					Resignation Status
				</InputLabel>
				<Select
					labelId="resignationStatus-multiple-checkbox-label"
					id="resignationStatus-multiple-checkbox"
					multiple
					name="resignationStatus"
					value={inputValue}
					onChange={handleChange}
					input={<OutlinedInput label="Resignation Status" />}
					renderValue={() =>
						inputValue
							.map((status) =>
								status
					...
					MenuProps={MenuProps}
				>
					{["recalled", "processing", "completed"].map((status, idx) => (
